...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((event, sess) => {
      ...
      if (event === "SIGNED_OUT" || (event === "INITIAL_SESSION" && !sess)) {
        // Atomic clear to prevent isInitialized flicker
        setSession(null);
        setProfile(null);
  ...
  useEffect(() => {
    ...
    if (!userId) {
      // Only clear if not already null to avoid redundant renders
      setProfile((prev) => (prev ? null : prev));
      setProfileReady((prev) => (prev ? false : prev));
      return;
    }
  ...
  const register = useCallback(
    async (form: RegisterForm, remember: boolean = true) => {
      const { fullName, motherName, email, password, village, landAreaHa } =
        form;
