...
function prettyLabel(raw: string): string {
  ...
  if (
    rawCat?.trim() === "TK Luar Keluarga" ||
    rawCat?.trim() === "TK Dalam Keluarga"
  ) {
...
export default function ReportScreen() {
  ...
  const canPrev =
    !yearActive && currentIdx >= 0 && currentIdx < orderedSeasons.length - 1;
    ...
    (async () => {
      ...
      try {
        ...
        setBase({
          ...
          laborNonCashNom: (d.labor || []).reduce(
            (a, r) => a + (r.value || 0),
            0,
          ),
  ...
  const showBlocking =
    (loadingService || profileLoading) && totalProduksi === 0;

  const musimLabel =
    seasonId && currentSeason
      ? `Musim-${currentSeason.season_no}`
      : "Semua Musim";

  const tahunLabel = yearActive
    ? String(year)
    : currentSeason
      ? `${currentSeason.season_year ? currentSeason.season_year : new Date(currentSeason.start_date).getFullYear()}`
      : "Semua Tahun";

  const pdfFileName = `Laporan ${musimLabel} ${tahunLabel}`;

  // ===== Row sederhana untuk Year Mode (tanpa Satuan & Harga) =====
  const SimpleRow = ({ label, value }: { label: string; value: number }) => (
  ...
  const yrRaw = (lbl: string) => {
    const row = yearRows.find(
      (r) => r.label.toLowerCase() === lbl.toLowerCase(),
    );
      ...
      >
            ...
            <Pressable
              onPress={async () => {
                await generateReportPdf({
                  ...
                  profileAreaHa,
                  effectiveArea:
                    effectiveArea != null ? effectiveArea : profileAreaHa,
                  landFactor,
                    ...
                    ? yearRows.map(
                        (r) =>
                          r.section === "rc"
                            ...
                            : {
                                ...r,
                                amount: Number(r.amount || 0) * landFactor,
                              }, // ⬅️ skala uang, bukan RC
                      )
                    : undefined,
                  share: true,
                });
        ...
        <View style={{ paddingHorizontal: S.spacing.lg }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {seasonId && currentSeason && (
                    <Text style={[styles.seasonRange, { color: C.textMuted }]}>
                      ...
                      {formatWithOutYear(currentSeason.end_date)}{" "}
                ...
                {orderedSeasons.map((s) => (
                  ...
                  >
                    ...
                    <Text style={{ color: C.textMuted, fontSize: 12 }}>
                      {formatWithOutYear(s.start_date)} —{" "}
                      {formatWithOutYear(s.end_date)} {`(${s.season_year})`}
                    </Text>
                  </Pressable>
                ))}
              </View>
          ...
          <Text style={[styles.tableTitle, { color: C.text, marginTop: 6 }]}>
            ...
            <Text style={{ color: C.danger, fontWeight: "900" as any }}>
              {profileAreaHa} Ha
            </Text>
          </Text>
            ...
            <>
              ...
              {yearActive ? (
                <View style={[styles.tableHead, { borderColor: C.border }]}>
                  <Text style={[styles.thUraian, { color: C.textMuted }]}>
                    Uraian
                  </Text>
                  <Text style={[styles.thRightSlim, { color: C.textMuted }]}>
                    Nilai (Rp)
                  </Text>
                </View>
              ) : (
                <View style={[styles.tableHead, { borderColor: C.border }]}>
                  ...
                  <Text style={[styles.thSmall, { color: C.textMuted }]}>
                    Jumlah
                  </Text>
                  <Text style={[styles.thSmall, { color: C.textMuted }]}>
                    Satuan
                  ...
                  <Text style={[styles.thRight, { color: C.textMuted }]}>
                    Nilai (Rp)
                  </Text>
                </View>
              )}
              ...
              {yearActive ? (
                <>
                  ...
                  <TotalLine
                    label="Total Biaya Non Tunai"
                    value={yrTotals.biayaNonTunai}
                    C={C}
                  />
                  <TotalLine
                    label="Total Biaya Tunai"
                    value={yrTotals.biayaTunai}
                    C={C}
                  />
                  <TotalLine
                    label="Total Biaya"
                    value={yrTotals.biayaTotal}
                    C={C}
                  />
                  ...
                  {mtList.map((n) => (
                    <React.Fragment key={`pend-mt-${n}`}>
                      ...
                      <SimpleRow
                        ...
                        value={yrMoney(
                          `Pendapatan Atas Biaya Non Tunai MT ${n}`,
                        )}
                      />
                      <SimpleRow
                  ...
                  <TotalLine
                    label="Total Pendapatan Atas Biaya Tunai"
                    value={yrTotals.pendTunai}
                    C={C}
                  />
                  <TotalLine
                    label="Total Pendapatan Atas Biaya Non Tunai"
                    value={yrTotals.pendNonTunai}
                    C={C}
                  />
                  <TotalLine
                    label="Total Pendapatan Atas Biaya Total"
                    value={yrTotals.pendTotal}
                    C={C}
                  />
                ...
                <>
                  ...
                  <TotalLine
                    label="Total Biaya Tunai"
                    value={totalBiayaTunai}
                    C={C}
                  />
                  <TotalLine
                    label="Total Biaya Non Tunai"
                    value={totalBiayaNonTunai}
                    C={C}
                  />
                  <TotalLine
                    label="Total Biaya"
                  ...
                  />
                  <TotalLine
                    label="R/C Biaya Tunai"
                    valueStr={rcTunai.toFixed(2)}
                    C={C}
                  />
                  <TotalLine
                    label="R/C Biaya Total"
                    valueStr={rcTotal.toFixed(2)}
                    C={C}
                  />
                </>
              )}
